...
export default function StopWatch({darkMode, setDarkMode, recordList, setRecordList, visibleButton, setVisibleButton}) {
    ...
    function deleteRecord(recordID) {
        console.log(recordList, recordID);

        const newRecords = recordList.filter((record) => record[0] != recordID);
            ...
            <div className="stopWatchDisplay">
                ...
                <div className="buttonContainer">
                    {visibleButton && <div className={"progressButtons"}>
                        ...
                        <button onClick={() => {setRecordList((prev) => [...prev, [recordID.current, timeHMS(time-prevStopWatch.current), timeHMS(time)]]); prevStopWatch.current = time; recordID.current += 1;}}>
                ...
                <div className = {"recordList " + (visibleButton ? '' : 'recordList-hide')}>
                    <table>
                        <thead>
                            <th class = {(darkMode ? 'dark' : 'light')}>No.</th>
                            <th class = {(darkMode ? 'dark' : 'light')}>LAB TIME</th>
                            <th class = {(darkMode ? 'dark' : 'light')}>TOTAL</th>
                            <th class = {(darkMode ? 'dark' : 'light')}></th>
                        </thead>
                        ...
                        <tbody>
                            {recordList.map((item, idx) =>(
                                <tr key={idx+1}>
                                    ...
                                    <td>{item[2]}</td>
                                    <td>
                                        <button class = {(darkMode ? 'dark' : 'light')} id = "deleteRecordBtn" onClick = {() => deleteRecord(item[0])}>
                                            ㅡ
                                        </button>
                                    </td>
                                </tr>  
